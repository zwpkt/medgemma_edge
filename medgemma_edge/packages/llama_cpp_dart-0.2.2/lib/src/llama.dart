...
// 在初始化函数中加载
      ...
      SamplerParams? samplerParams)
  {
    ...
    if (_verbose == false) {
      ...
      Pointer.fromFunction<LlamaLogCallback>(_myLogCallback);
    ...
    if (!modelFile.existsSync()) {
      throw LlamaException("❌ 模型文件不存在: $modelPath");
    }
    final fileSize = modelFile.lengthSync();
    print("📁 模型文件大小: ${(fileSize / 1024 / 1024).toStringAsFixed(2)} MB");

    // 检查文件头（GGUF 格式）
    try {
      final bytes = modelFile.openSync();
      final header = bytes.readSync(4);
      bytes.closeSync();
      final headerStr = String.fromCharCodes(header);
      print("📄 文件头: $headerStr (应为 GGUF)");
      if (headerStr != "GGUF") {
        print("⚠️ 警告：文件可能不是有效的 GGUF 格式");
      }
    } catch (e) {
      print("⚠️ 无法读取文件头: $e");
    }
    ...
    try {
      print("🔄 调用 llama_load_model_from_file...");
      print("📊 模型参数:");
      print("   n_gpu_layers: ${modelParams.n_gpu_layers}");
      print("   use_mmap: ${modelParams.use_mmap}");
      print("   use_mlock: ${modelParams.use_mlock}");
      print("   vocab_only: ${modelParams.vocab_only}");
      print("   main_gpu: ${modelParams.main_gpu}"); // 确保这里是 -1
      ...
      print("⏱️ 加载耗时: $loadTime ms");

      if (loadedModel == nullptr) {
        // 尝试获取错误信息
        String errorMsg = "Could not load model at $modelPath";
        throw LlamaException(errorMsg);
      }
      ...
      print("❌ 加载异常: $e");
      if (loadedModel != nullptr) {
        lib.llama_free_model(loadedModel);
      }
      rethrow;
    } finally {
    ...
    print("🔄 调用 llama_new_context_with_model...");
    contextParamsDart ??= ContextParams();
    _contextParams = contextParamsDart;
    _nPredict = contextParamsDart.nPredict;
    print("...start to get contextParams:") ;
    var contextParams = contextParamsDart.get();
    if(contextParams == null){
      print("📊 上下文参数: null");
      throw LlamaException("contextParams is null");
    }else{
      print("📊 上下文参数:");
      print("   n_ctx: ${contextParams.n_ctx}");
    }
    ...
    if (mmprojPath != null && mmprojPath.isNotEmpty) {
      ...
      if(mprojPathPtr==null){
        if (loadedContext != nullptr) lib.llama_free(loadedContext);
        lib.llama_free_model(model);
        throw LlamaException("mmprojPath.toNativeUtf8().cast<Char>() is null");
      }
  ...
  // 自定义日志回调
  static void _myLogCallback(int level, Pointer<Char> text, Pointer<Void> userData) {
    final message = text.cast<Utf8>().toDartString();
    if (level == 2) { // 只打印错误
      print("🔴 [llama error] $message");
    } else if (level == 3) { // 打印警告
      print("🟡 [llama warn] $message");
    } else {
      print("📢 [llama] $message");
    }
  }
